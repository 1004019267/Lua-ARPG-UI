...
namespace LuaFramework
{
    ...
    public class ResourceManager : Manager
    {
        ...
        public void LoadPrefab2(string abName, string[] assetNames, LuaFunction func)
        {
            abName = "file://" + Application.streamingAssetsPath + "/" + abName;
            LoadAsset<GameObject>(abName, assetNames, null, func);
        }
...
namespace LuaFramework
{
    public class ResourceManager : MonoBehaviour
    {
        ...
        /// 载入素材
        /// </summary>
        public T LoadAsset<T>(string path, string abname, string assetname) where T : UnityEngine.Object
        {
            abname = abname.ToLower();
            AssetBundle bundle = LoadAssetBundle(path, abname);
            T t = bundle.LoadAsset<T>(assetname);
            return t; 
        }

        public string LoadConfig(string path, string abname, string assetname)
        {
            TextAsset txt = LoadAsset<TextAsset>(path, abname, assetname);
            return txt.text;
        }


        public Sprite LoadSprite(string path, string abname, string assetname)
        {
            Texture2D tex = LoadAsset<Texture2D>(path, abname, assetname);
            Sprite sprite = Sprite.Create(tex, new Rect(0, 0, tex.width, tex.height), Vector2.zero);
            return sprite;           
        }  
        public RenderTexture LoadRenderTexture(string path, string abname, string assetname)
        {
            RenderTexture tex = LoadAsset<RenderTexture>(path, abname, assetname);
            return tex;
        }
        ...
        public void LoadPrefab(string path, string abName, string[] assetNames, LuaFunction func)
        {
            abName = abName.ToLower();
            List<UObject> result = new List<UObject>();
            for (int i = 0; i < assetNames.Length; i++)
            {                
                UObject go = LoadAsset<UObject>(path, abName, assetNames[i]);
        ...
        /// <param name="abname"></param>
        /// <returns></returns>
        public AssetBundle LoadAssetBundle(string path, string abname)
        {
            if (!abname.EndsWith(AppConst.ExtName))
            {
                abname += AppConst.ExtName;
            ...
            {
                byte[] stream = null;
                string uri = Util.DataPath + path + "/" + abname;
                Debug.LogWarning("LoadFile::>> " + uri);
                LoadDependencies(path, abname);
        ...
        void LoadDependencies(string path, string name)
        {
            if (manifest == null)
            {
            ...
            for (int i = 0; i < dependencies.Length; i++)
            {
                LoadAssetBundle(path, dependencies[i]);
            }
        }
