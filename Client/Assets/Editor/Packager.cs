...
public class Packager
{
    ...
    static void AddSingleBuildMap(string path, string pattern)
    {
        string[] files = Directory.GetFiles(path, "*" + pattern);
        if (files.Length == 0) return;
        ...
        for (int i = 0; i < files.Length; i++)
        {
            ...
            AssetBundleBuild build = new AssetBundleBuild();
            string str = files[i].Remove(0, 17);
            build.assetBundleName = str.Replace(pattern, AppConst.ExtName);
            build.assetNames = new string[] { files[i] };
            maps.Add(build);
        }
    }

    static void AddLuaBuildMap(string path, string pattern)
    {
        string[] files = Directory.GetFiles(path, "*" + pattern);
        if (files.Length == 0) return;
        ...
        for (int i = 0; i < files.Length; i++)
        {
            ...
            AssetBundleBuild build = new AssetBundleBuild();
            string str = files[i].Remove(0, 15);
            build.assetBundleName = str.Replace(pattern, AppConst.ExtName);
            build.assetNames = new string[] { files[i] };
            maps.Add(build);
        }
    ...
    static void HandleLuaBundle()
    {
        ...
        for (int i = 0; i < dirs.Length; i++)
        {
            ...
            //AddLuaBuildMap(path, "*.bytes");
        }
        AddBuildMap("lua/lua" + AppConst.ExtName, "*.bytes", "Assets/" + AppConst.LuaTempDir);
        //AddLuaBuildMap("Assets/" + AppConst.LuaTempDir, "*.bytes");
    ...
    static void HandleExampleBundle()
    {
        ...
        AddSingleBuildMap("Assets/Resources/Units", ".prefab");
        AddSingleBuildMap("Assets/Resources/UI", ".prefab");
        AddSingleBuildMap("Assets/Resources/Config", ".xml");
        AddSingleBuildMap("Assets/Resources/FX", ".prefab");
        AddSingleBuildMap("Assets/Resources/Icon", ".png");
        AddSingleBuildMap("Assets/Resources/Render", ".renderTexture");         
    }
    ...
    static void HandleLuaFile()
    {
        ...
        string[] luaPaths = { AppDataPath + "/lua/",  AppDataPath + "/Tolua/Lua/" };
